...
const rigidBody = [{x: SIZE * 10, y: SIZE * 10, w: SIZE * 10, h: SIZE * 2, a: 0,},]
const body = {
...
const main = () => setInterval(() => {
  ...
  if (key.s.flag) body.waist.dx -= moveAccelaration * internalDiffTime
  if (key.f.flag) body.waist.dx += moveAccelaration * internalDiffTime
  if (key.e.flag) body.waist.dy -= moveAccelaration * internalDiffTime
  if (key.d.flag) body.waist.dy += moveAccelaration * internalDiffTime
  // collision detect
  wallVertex.forEach((vn, i) => {
    ...
    const d = {
      x: body.waist.x + body.waist.dx * internalDiffTime,
      y: body.waist.y + body.waist.dy * internalDiffTime,
    }
    const cdx = d.x - c.x
    const cdy = d.y - c.y
  ...
  body.waist.dy += gravity * internalDiffTime
  rigidBody.forEach(v => {
    v.a += PI * .0001 * internalDiffTime
  })
}, 0)
const draw = () => {
  ...
  ctx.fillRect(SIZE * 10, 0, SIZE, SIZE)
  ctx.beginPath()
  rigidBody.forEach(v => {
    const w = v.w / 2
    const h = v.h / 2
    const c = Math.cos(v.a)
    const s = Math.sin(v.a)
    ctx.arc(v.x, v.y, SIZE / 2, 0, PI * 2)
    ctx.moveTo(v.x + w * c + h * s, v.y + w * s - h * c)
    ctx.lineTo(v.x + w * c - h * s, v.y + w * s + h * c)
    ctx.lineTo(v.x - w * c - h * s, v.y - w * s + h * c)
    ctx.lineTo(v.x - w * c + h * s, v.y - w * s - h * c)
  })
  Object.values(body).forEach(v => {
