...
const slideProcess = () => {
  ...
  if (slide.state === 'pullBack' && slide.stop * inventory[0].slideSpeed <= slide.time) {
    ...
    const stopSlide = () => {
      ...
      if ((0 < key[action.combatReload] || combatReload.auto === 'ON') && combatReload.magFlag) {
        combatReload.flag = true
        combatReload.weight = (key[action.combatReload] === 0) ? 8 : 4
      }
    }
    stopSlide()
...
const dashProcess = () => {
  if (cloneDashType1Flag && key[action.slow] === 0) cloneSpeed = dash.breakthrough
  else if (cloneDashType2Flag) { ownSpeed.current = dash.breakthrough
  } else if (cloneDashType3Flag && key[action.slow] === 0) {
    clonePosition = []
...
const moving = () => {
  ...
  if (cloneFlag) {
    if (0 < key[action.slow]) {
      ...
      if (ownSpeed.max < cloneSpeed && (cloneDashType1Flag || cloneDashType2Flag || cloneDashType3Flag)) {
...
const interfaceProcess = () => {
  if (key[action.pause] === 1) state = 'pause'
  if (key[action.inventory] === 1) inventoryFlag = !inventoryFlag
  speedAdjust()
  if (0 < key[action.lookUp]) angle = (angle+1)|0
  if (0 < key[action.lookRight]) angle = (angle+2)|0
  if (0 < key[action.lookDown]) angle = (angle+4)|0
  if (0 < key[action.lookLeft]) angle = (angle+8)|0
  if (0 < key[action.up]) direction = (direction+1)|0
  if (0 < key[action.right]) direction = (direction+2)|0
  if (0 < key[action.down]) direction = (direction+4)|0
  if (0 < key[action.left]) direction = (direction+8)|0
  if (0 < angle) currentDirection = angle
  else if (direction !== 0) currentDirection = direction
  if (key[action.reload] === 1 && reload.time === 0 && reload.state === 'done') {
    inventory[0].reloadSpeed = inventory[0].baseReloadSpeed
    reloadProcess()
  } else if (0 < reload.time || reload.state !== 'done') reloadProcess()
  if (key[action.fire]) firingProcess()
  loadingProcess()
  if (key[action.change] === 1) magazineForword() // TODO: to consider
  if (dashCoolTimer() === 0 && key[action.dash] === 1) dashProcess()
  if (0 < direction || ownSpeed.max < ownSpeed.current || ownSpeed.max < cloneSpeed) moving()
  if (key[action.debug] === 1) debugMode = !debugMode
  if (manyAmmo() && key[action.back] === 1) bomb()
}
const cloneProcess = () => {
  if (
...
const setOwnImage = arg => {
  return (arg === 1) ? './img/TP2U.png'
  : (arg === 3) ? './img/TP2RU.png'
  : (arg === 2) ? './img/TP2R.png'
  : (arg === 6) ? './img/TP2RD.png'
  : (arg === 4) ? './img/TP2D.png'
  : (arg === 12) ? './img/TP2LD.png'
  : (arg === 8) ? './img/TP2L.png'
  : (arg === 9) ? './img/TP2LU.png'
  : './img/TP2F.png'
}
...
const drawEnemies = () => {
  enemies.forEach(enemy => {
    ...
    if (
      ...
      ? {F: './img/JK32F.png', L: './img/JK32L.png', R: './img/JK32R.png'}
      : (enemy.imageID === 1)
      ? {F: './img/JK33F.png', L: './img/JK33L.png', R: './img/JK33R.png'}
      : (enemy.imageID === 2)
      ? {F: './img/JK34F.png', L: './img/JK34L.png', R: './img/JK34R.png'}
      : {F: './img/JK35Fv1.png', L:'./img/JK35Fv1.png', R: './img/JK35Fv1.png'}
...
const bulletProcess = () => {
  if (homingFlag) {
    ...
    bullets.forEach((bullet, i) => {
      ...
      let theta = (0 < key[action.combatReload]) ?  -degree * (Math.PI / 180)
      : (0 < key[action.change]) ? degree * (Math.PI / 180) : 0
      ...
      if (explosive2Flag) {
        ...
        } else if (bullet.prepareFlag && key[action.fire] === 1) bullet.life = 2
        else if (key[action.fire] === 1) bullet.prepareFlag = true
        return false
      }
      ...
      if (explosive3Flag) {
        ...
        } else if (bullet.prepareFlag && key[action.fire] === 1) {
          bullet.life = 2
          return
        }
        else if (key[action.fire] === 1) bullet.prepareFlag = true
...
const inventoryProcess = () => {
  ...
  selectedIndex = (key[action.lookUp] === 1) ? 1
  : (key[action.lookRight] === 1) ? 2
  : (key[action.lookDown] === 1) ? 3
  : (key[action.lookLeft] === 1) ? 4 : selectedIndex
  const swapArray = (pressedKey, num) => {
    if (num + 1 <= inventory.length && pressedKey === 1 && afterglow.inventory === 0) {
      if (0 < key[action.slow]) {
  ...
  if (selectedBuffer === 1 || 0 < key[action.slow]) swapArray(key[action.lookUp], 1)
  if (selectedBuffer === 2 || 0 < key[action.slow]) swapArray(key[action.lookRight], 2)
  if (selectedBuffer === 3 || 0 < key[action.slow]) swapArray(key[action.lookDown], 3)
  if (selectedBuffer === 4 || 0 < key[action.slow]) swapArray(key[action.lookLeft], 4)
...
const upgradeOne = () => {
  if (key[action.lookUp] === holdTimeLimit && inventory[0].offensivePower <= ammo) {
...
const upgradeDash =() => {
  ...
  if (key[action.lookRight] === holdTimeLimit && cost.dashDistance <= point) {
  ...
  if (key[action.lookDown] === holdTimeLimit && cost.dashCooltime <= point) {
  ...
  if (key[action.lookLeft] === holdTimeLimit && cost.dashDistance <= point) {
...
const upgradeExplosive = () => {
  if (key[action.lookDown] === holdTimeLimit && inventory[0].explosive3 <= ammo) {
    homingFlag = false
...
const upgradeTest = () => {
  if (key[action.lookRight] === holdTimeLimit && cost.clone <= point) {
    cloneFlag = true
...
const upgradeLimitBreak = () => {
  if (key[action.lookUp] === holdTimeLimit && inventory[0].limitBreak <= point) {
  ...
  if (key[action.lookLeft] === holdTimeLimit && inventory[0].reset <= point) {
...
const upgradeClone = ()  => {
  if (key[action.lookUp] === holdTimeLimit) {
  ...
  if (key[action.lookRight] === holdTimeLimit) {
  ...
  if (key[action.lookDown] === holdTimeLimit) {
    cloneDashType1Flag = false
    cloneDashType2Flag = false
    cloneDashType3Flag = !cloneDashType3Flag
    afterglow.explosive3 = holdTimeLimit
  } else if (0 < afterglow.explosive3) afterglow.explosive3 = (afterglow.explosive3-1)|0
  if (key[action.lookLeft] === holdTimeLimit) {
...
const drawStore = () => {
  ...
  objects.forEach((object,index) => { // only rectangle
    ...
    && object.y < ownPosition.y - canvas.offsetHeight/2) { // left & top
      ...
      context.save()
      imgStore.src = (object.ID === 0) ? './img/st1v2.png'
      : (object.ID === 1) ? './img/st2v1.png'
      : './img/stv1.png'
      if (index === 6) {
        context.scale(1, 1)
    ...
    const topColumn = (effect, expence, material, image) => {
      ...
      : (expence <= material && key[action.lookUp] <= holdTimeLimit)
      ? `hsl(0, 50%, ${50 + 50 * key[action.lookUp] / holdTimeLimit}%)`
      ...
      context.fillText(
        `${expence} ${text}`,
        relativeX(object.x + size * 2.5), relativeY(object.y - size * 2)
      )
      context.restore()
      if ((expence <= material && key[action.lookUp] <= holdTimeLimit) || 0 < effect) {
        ratio = (effect === 0) ? key[action.lookUp] / holdTimeLimit : 1
        context.fillRect(
          relativeX(object.x - size * 3.5),
          relativeY(object.y - size * 3.5), ratio * size * 12, size / 2
        )
      }
    }
    const rightColumn = (effect, expence, material, image) => {
      ...
      : (expence <= material && key[action.lookRight] <= holdTimeLimit)
      ? `hsl(280, 50%, ${50 + 50 * key[action.lookRight] / holdTimeLimit}%)`
      ...
      context.fillText(
        `${expence} ${text}`,
        relativeX(object.x+size * 6), relativeY(object.y+size * 4.5)
      )
      if ((expence <= material && key[action.lookRight] <= holdTimeLimit) || 0 < effect) {
        ratio = (effect === 0) ? key[action.lookRight] / holdTimeLimit : 1
        context.fillRect(
          relativeX(object.x+size * 6), relativeY(object.y+size * 3.5),
          ratio * size * 12, size / 2
        )
      }
    }
    const downColumn = (effect, expence, material, image) => {
      ...
      : (expence <= material && key[action.lookDown] <= holdTimeLimit)
      ? `hsl(340, 50%, ${50 + 50 * key[action.lookDown] / holdTimeLimit}%)`
      ...
      context.fillText(
        `${expence} ${text}`,
        relativeX(object.x + size * 2.5), relativeY(object.y+size * 10)
      )
      context.restore()
      if ((expence <= material && key[action.lookDown] <= holdTimeLimit) || 0 < effect) {
        ratio = (effect === 0) ? key[action.lookDown] / holdTimeLimit : 1
        context.fillRect(
          relativeX(object.x - size * 3.5), relativeY(object.y+size * 8.5),
          ratio * size * 12, size / 2
        )
      }
    }
    const leftColumn = (effect, expence, material, image) => {
      ...
      : (expence <= material && key[action.lookLeft] <= holdTimeLimit)
      ? `hsl(100, 50%, ${50 + 50 * key[action.lookLeft] / holdTimeLimit}%)`
      ...
      const text = (material === ammo) ? 'TAPIOCA' : 'POINT'
      context.fillText(
        `${expence} ${text}`, relativeX(object.x-size * 1), relativeY(object.y+size * 4.5)
      )
      context.restore()
      if ((expence <= material && key[action.lookLeft] <= holdTimeLimit) || 0 < effect) {
        ratio = (effect === 0) ? key[action.lookLeft] / holdTimeLimit : 1
        context.fillRect(
          relativeX(object.x-size * 12), relativeY(object.y+size * 3.5),
          ratio * size * 12, size / 2
        )
      }
    }
    if ((
      ...
      if (index === 0) {
        topColumn(afterglow.offensivePower, inventory[0].offensivePower, ammo, './img/TASTEv1.jpg')
        if (homingFlag) rightColumn(afterglow.homing, inventory[0].homing, ammo, './img/Homingv1.jpg')
        else rightColumn(afterglow.homing, inventory[0].homing, ammo, './img/Homingv1.jpg')
        downColumn(afterglow.slide, inventory[0].slide, point, './img/EASY_TO_DRINKV1.jpg')
        leftColumn(afterglow.bulletSpeed, inventory[0].bulletSpeedCost, point, './img/TAPIOCA_SPEEDv1.png')
      } else if (index === 1) {
        topColumn(afterglow.magazine, inventory[0].magazine, point, './img/CUP_SIZEv1.png')
        rightColumn(afterglow.magAmount, inventory[0].magAmount, point, './img/CUP_AMOUNTv1.png')
        downColumn(afterglow.loading, inventory[0].loading, point, './img/COOKING_TIMEv1.png')
        leftColumn(afterglow.ammo, ammoLog, ammo, './img/TAPIOCA_CAPACITYv1.png')
      } else if (index === 2) {
        topColumn(afterglow.dashDamage, cost.dashDamage, ammo, './img/JK34F.png')
        context.fillText(
          'DASH ATTACK DAMAGE +',
          relativeX(object.x - size * 4), relativeY(object.y - size * 3.5)
        )
        rightColumn(afterglow.dashDistance, cost.dashDistance, point, './img/JK34R.png')
        context.fillText(
          'DASH DISTANCE +',
          relativeX(object.x + size * 6), relativeY(object.y + size * 3)
        )
        downColumn(afterglow.dashCooltime, cost.dashCooltime, point, './img/JK34F.png')
        context.fillText(
          'DASH COOLTIME -',
          relativeX(object.x - size * 2.5), relativeY(object.y + size * 8.5)
        )
        leftColumn(afterglow.dashSpeed, cost.dashDistance, point, './img/JK34L.png')
        context.fillText(
          'DASH SPEED +',
          relativeX(object.x-size * 9.5), relativeY(object.y + size * 3)
        )
      } else if (index === 3) {
        topColumn(afterglow.bulletLife, inventory[0].bulletLifeCost, point, './img/JK32F.png')
        context.fillText(
          'SHELF LIFE +', relativeX(object.x + size * 2), relativeY(object.y - size * 3.5)
        )
        if (!cloneFlag) {
          rightColumn(afterglow.clone, cost.clone, point, './img/JK33R.png')
          context.fillText(
        ...
        topColumn(afterglow.explosive1, inventory[0].explosive1, ammo, './img/TP2F.png')
        context.fillText(
          'EXPLOSIVE TYPE I',
          relativeX(object.x - size * 4), relativeY(object.y - size * 3.5)
        )
        rightColumn(afterglow.explosive2, inventory[0].explosive2, ammo, './img/TP2F.png')
        context.fillText(
          'EXPLOSIVE TYPE II',
          relativeX(object.x+size * 6), relativeY(object.y+size * 3)
        )
        downColumn(afterglow.explosive3, inventory[0].explosive3, ammo, './img/TP2F.png')
        context.fillText(
          'EXPLOSIVE TYPE III',
          relativeX(object.x+size * 2.5), relativeY(object.y+size * 8.5)
        )
        leftColumn(afterglow.explosiveRange, inventory[0].explosiveRange, point, './img/TP2F.png')
        context.fillText(
          'EXPLOSIVE RANGE +',
          relativeX(object.x-size * 9.5), relativeY(object.y+size * 3)
        )
      } else if (index === 5) {
        topColumn(afterglow.limitBreak, inventory[0].limitBreak, point, './img/JK32F_O1.png')
        context.fillText(
          'RIMIT BREAK CHANCE',
          relativeX(object.x + size * 2), relativeY(object.y - size * 3.5)
        )
        leftColumn(afterglow.reset, cost.reset, point, './img/JK32F.png')
        context.save()
        context.textAlign = 'right'
        ...
        topColumn(afterglow.explosive1, cost.reset, point, './img/JK34F.png')
        context.fillText(
          'CLONE DASH TYPE I',
          relativeX(object.x - size * 4), relativeY(object.y - size * 3.5)
        )
        rightColumn(afterglow.explosive2, cost.reset, point, './img/JK34R.png')
        context.fillText(
          'CLONE DASH TYPE II',
          relativeX(object.x+size * 6), relativeY(object.y+size * 3)
        )
        downColumn(afterglow.explosive3, cost.reset, point, './img/JK34F.png')
        context.fillText(
          'CLONE DASH TYPE III',
          relativeX(object.x+size * 2.5), relativeY(object.y+size * 8.5)
        )
        leftColumn(afterglow.explosiveRange, cost.reset, point, './img/JK34L.png')
        context.fillText(
          'CLONE COME BUCK',
          relativeX(object.x-size * 9.5), relativeY(object.y+size * 3)
        )
...
const command = () => {
  ...
  let counter = 0
  return () => {
    if (counter % 2 === 0 && 0 < key[action.lookUp] && 0 < key[action.up]) counter += 1
    else if (counter % 2 === 1 && 0 < key[action.lookUp] && 0 < key[action.down]) counter += 1
    if (counter === 5) {
      bool = true
    }
    if (state === 'result') {
...
const reset = () => {
  ...
  ammo = 24
  cartridgeInfo = {life: 100, speed: 1}
  loading = {
    time: 0,
    takeOut: 15,
    takeOutFlag: false,
...
const drawTitleScreen = () => {
  ...
  drawCharacter('./img/JK32F.png', c.x, c.y)
  if (ss % 2 === 1 && ~~(ms/100) === 0) c.y = c.y + size/16
  if (ss % 2 === 1 && ~~(ms/100) === 5) c.y = c.y - size/16
  drawCharacter('./img/JK33F.png', c.x + size * 2, c.y)
  if (ss % 2 === 1 && ~~(ms/100) === 5) c.y = c.y + size/16
  if (ss % 2 === 0 && ~~(ms/100) === 0) c.y = c.y - size/16
  drawCharacter('./img/JK34F.png', c.x + size * 4, c.y)
  if (ss % 2 === 0 && ~~(ms/100) === 0) c.y = c.y + size/16
  if (ss % 2 === 0 && ~~(ms/100) === 5) c.y = c.y - size/16
  drawCharacter('./img/JK35Fv1.png', c.x + size * 6, c.y)
}
const titleProcess = () => {
  resetScreen()
  if (key[action.fire] === 1) {
    reset()
    if (manyAmmo()) {
  ...
  if (key[action.slow] === 1) {
    input()
    state = 'keyLayout'
  }
  if (key[action.change] === 1) mapMode = !mapMode
  drawTitleScreen()
}
const mainProcess = () => {
  resetScreen()
...
const pauseProcess = () => {
  if (key[action.pause] === 1) state = 'main'
  resetScreen()
  let nowTime = Date.now()
  let ss = ('0' + ~~(nowTime % 6e4 / 1e3)).slice(-2)
  let ms = ('0' + ~~(nowTime % 1e3)).slice(-3)
...
const resultProcess = () => {
  ...
  let nowTime = Date.now()
  let ss = ('0' + ~~(nowTime % 6e4 / 1e3)).slice(-2)
  imgCutin.src = (ss % 3 === 0) ? './img/drinking.png' : './img/drinkSmile.png'
  context.scale(3, 3)
  context.drawImage(
    imgCutin,
...
const menuColumn = () => {
  ...
  let num = array[0]
  return () => {
    if (key[action.down] === 1) {
      if (num === array.slice(-1)[0]) num = array[0]
      else num = (num+1)|0
    } else if (key[action.up] === 1) {
...
const keyLayoutProcess = () => {
  ...
  if (rowNum === 0 && (key[action.left] === 1 || key[action.right] === 1)) {
    reload.auto = (reload.auto === 'ON') ? setStorage('autoReload', 'OFF')
    : setStorage('autoReload', 'ON')
  }
  if (rowNum === 1 && (key[action.left] === 1 || key[action.right] === 1)) {
  ...
  context.textAlign = 'right'
  context.fillStyle = (
      inKey === order.indexOf(action.back) && key[action.back] === 0
  ...
  if (inKey === order.indexOf(action.back)) {
    context.fillRect(
      ...
      size * 10.6 * key[action.back] / holdTimeLimit, size * .2
    )
  }
}
const loop = () => {
...
const imagePathList = [
  './img/TP2F.png', './img/TP2U.png', './img/TP2RU.png',
  './img/TP2R.png', './img/TP2RD.png', './img/TP2D.png',
  './img/TP2LD.png', './img/TP2L.png', './img/TP2LU.png',
  './img/JK32F.png', './img/JK32L.png', './img/JK32R.png',
  './img/JK32F_O1.png', './img/JK32F_O2.png',
  './img/JK33F.png', './img/JK33L.png', './img/JK33R.png',
  './img/JK34F.png', './img/JK34L.png', './img/JK34R.png',
  './img/JK35Fv1.png',
  './img/drinkSmile.png', './img/drinking.png',
  './img/ROGOv1.png', './img/ROGOv1.2.png',
  './img/stv1.png', './img/st1v2.png', './img/st2v1.png',
  './img/Homingv1.jpg', './img/TAPIOCA_PENETRATEv1.png',
  './img/TASTEv1.jpg', './img/EASY_TO_DRINKV1.jpg' , './img/TAPIOCA_SPEEDv1.png',
  './img/TAPIOCA_PENETRATEv1.png', './img/COOKING_TIMEv1.png', './img/CUP_AMOUNTv1.png',
  './img/CUP_SIZEv1.png', './img/TAPIOCA_CAPACITYv1.png',
  './img/arrowDown.png', './img/arrowRight.png', './img/arrowUp.png'
]
let loadedList = []
let loadedMap = []
