...
let time
let mouseFlag = false
let mouseCooldinateObject = {x: 0, y: 0}
...
const shot = () => {
  ...
  const info = {
    x: canvas.offsetWidth / 2 - mouseCooldinateObject.x,
    y: canvas.offsetHeight / 2 - mouseCooldinateObject.y
  }
...
const main = () => {
  ...
  const corsor = () => {
    context.beginPath()
    context.arc(mouseCooldinateObject.x, mouseCooldinateObject.y, size / 2, 0, Math.PI * 2, false)
